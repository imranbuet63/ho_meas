  ...
  if (UL_info->rach_ind.number_of_pdus > 0 && in_timewindow) {
    ...
    for (int i = 0; i < UL_info->rach_ind.number_of_pdus; i++) {
      ...
      if (UL_info->rach_ind.pdu_list[i].num_preamble > 0) {
        LOG_I(NR_MAC,"UL_info->rach_ind.pdu_list[%d].preamble_list[%d].preamble_pwr = %d\n",i,0,UL_info->rach_ind.pdu_list[i].preamble_list[0].preamble_pwr);
        if (UL_info->rach_ind.pdu_list[i].preamble_list[0].preamble_pwr < 90000) {
          continue;
        }
        LOG_I(NR_MAC,"preamble_index = %d timing advance = %d\n",UL_info->rach_ind.pdu_list[i].preamble_list[0].preamble_index,UL_info->rach_ind.pdu_list[i].preamble_list[0].timing_advance);
      }

      if (UL_info->rach_ind.pdu_list[i].num_preamble != 1){
        LOG_I(NR_MAC,"Num preambles > 1 not handled yet\n");
        continue;
      }
