                                           ...
                                           uint32_t *data_len)
{
  ...
  for (int i = 0; i < pNfapiMsg->Number_of_PDUs; i++) {
    ...
    // remove from PDULength 12 bytes (leave only the size occupied in the payload buffer afterward)
...
int fapi_nr_pack_and_send_p7_message(vnf_p7_t *vnf_p7, nfapi_p7_message_header_t *header)
{
  ...
  if (header->message_id
    ...
    int len_FAPI = aerial_pack_tx_data_request(header,
                                               ...
                                               &data_len);
    return test_nv_ipc_send_msg_P7_with_data(FAPI_buffer, len_FAPI, FAPI_data_buffer, data_len, header);
  } else {
    ...
    return test_nv_ipc_send_msg_P7(FAPI_buffer, len_FAPI, header);
  }
}
